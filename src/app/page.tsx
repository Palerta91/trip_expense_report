    ...
    <AppShell userName={user.name} userRole={user.role}>
      ...
      <section className="quick-actions-section" aria-labelledby="quick-actions-heading">
        ...
        <div className="quick-actions">
          ...
          {activeTrip ? <Link className="quick-action expense-action" href={`/trips/${activeTrip.id}/expenses/new`}><span className="quick-action-icon"><WalletCards size={25} /></span><span><strong>Добавить статью расхода</strong><small>Внесите данные вручную, без чека и ИИ</small></span><ArrowUpRight size={20} /></Link> : <div className="quick-action disabled"><span className="quick-action-icon"><WalletCards size={25} /></span><span><strong>Добавить статью расхода</strong><small>Сначала откройте командировку</small></span></div>}
        </div>
      </section>
