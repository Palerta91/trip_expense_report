  ...
  return (
    <AppShell userName={user.name}>
      <section className="card trip-hero">
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><Link className="button secondary" href={`/api/trips/${tripId}/report?format=xlsx`}><FileDown size={17} />Excel</Link><Link className="button secondary" href={`/api/trips/${tripId}/report?format=docx`}><FileDown size={17} />Word</Link>{(user.role === "ADMIN" || result.trip.managerId === user.id) && <Link className="button secondary" href={`/trips/${tripId}/members`}>Участники</Link>}<Link className="button secondary" href={`/trips/${tripId}/receipts/new`}><FileUp size={17} />Загрузить чек</Link><Link className="button" href={`/trips/${tripId}/expenses/new`}><Plus size={17} />Добавить расход</Link>{user.role === "ADMIN" && <DeleteTripButton tripId={tripId} title={result.trip.title} />}</div>
      </section>
